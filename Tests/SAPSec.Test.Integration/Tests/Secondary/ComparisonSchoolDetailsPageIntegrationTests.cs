    ...
    ITestOutputHelper outputHelper) : InMemoryRepositoryIntegrationTests(fixture, outputHelper)
{
    ...
    public override Task InitializeAsync()
    {
        Fixture.EstablishmentRepository.SetupEstablishments(
            Build.Establishment(CurrentSchoolUrn, "Test School 1", x => x.Open().Secondary().InLA("001")),
    ...
    public async Task HeadingAndTitle_ReflectCurrentAndComparatorSchools()
    {
        var page = await Fixture.RequestPageAsync(
            Routes.SecondarySchool(CurrentSchoolUrn).Comparison(SimilarSchoolUrn).SchoolDetails);
