    ...
    : EndToEndTests(fixture)
{
    ...
    public override async Task InitializeAsync()
    {
        ...
        await Expect(Page).ToHaveURLAsync(AllThroughSchoolRoute.KS4HeadlineMeasures);
    }
    ...
    [Fact]
    public async Task Attainment8_ViewAndNavigateToTopPerfomers()
    {
        var section = await GetSection(Attainment8HeaderText);
        ...
        await Expect(Page).ToHaveURLAsync(new Regex(AllThroughSchoolRoute.SecondaryComparison(UrlPattern).Similarity));
        ...
        await Expect(Page).ToHaveURLAsync($"{AllThroughSchoolRoute.ViewSimilarSchools}?phase=secondary");
    }
