...
{
    public async Task<SimilarSchoolsData<Ks4PerformanceData>> GetSimilarSchoolsData(string currentSchoolUrn)
    {
...
{
    public async Task<SimilarSchoolsData<Ks4DestinationsData>> GetSimilarSchoolsData(string currentSchoolUrn)
    {
        var similarSchoolUrns = (await similarSchoolsRepository.GetGroupAsync(currentSchoolUrn))
            .Select(g => g.NeighbourURN)
            .Where(urn => !string.IsNullOrWhiteSpace(urn))
